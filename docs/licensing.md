...
2026-09-21 Windows 补验：安装包构建、隔离 NSIS 安装/卸载、安装后引擎与 120 项授权真进程检查已通过；修复了 asar 入口误认与异步到期事件测试。范围、日志、随包声明遗漏和剩余交付项见 [Windows 验收记录](windows-commercial-acceptance-2026-09-21.md)。下列原先的 Windows/安装包空白现已部分补齐，正式身份、干净电脑与真实任务仍未完成。

- **Windows 剩余边界。** 常规授权落盘、`taskkill` 停守护、安装后 `e2e:license`、带一次性公钥的构建与 asar 检查已实跑；杀毒软件占用文件等故障注入、干净 Windows VM 仍待验证。
- **正式安装包。** `package:win` 已用一次性身份出包，隔离标识包已安装/卸载并验证包内授权链；`package:mac` 仍未做。
  "新电脑用正式公钥安装包导入客户授权 → 配 Key → 真实板级任务"没有做。
