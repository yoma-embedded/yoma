 ...
 *
 * 器件数据包(irpack)是 CubeMX 器件库的解析产物,不进 git,但 --dist 会随包交付。显式允许缺数据的构建
 * 可能只有内核二进制、没有数据。所以 `schema` 之外的命令在那样的机器上一个都跑不了,话要说
...
export function noDataPacksMessage(command: string): string {
  return (
    `stm32config ${command}: no STM32 device data packs (irpacks) are installed in this build. Use an engine distribution containing data/stm32/*.irpack; source builders can import them from STM32CubeMX with npm run engines:build. ` +
...
export function describeCoverage(options?: EnginePathOptions): string {
  try {
    ...
    return "Coverage on this machine: no device data packs installed — only `schema` works; use the datasheet tool for pin / peripheral facts."
  }
}
