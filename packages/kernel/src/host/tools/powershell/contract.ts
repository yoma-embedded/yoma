...
- The script is passed as -EncodedCommand, preserving its quotes, semicolons and backslashes. Before it runs, the working directory is set using -LiteralPath, progress records are suppressed, and both console output and native-program stdin use UTF-8.
...
- Use -LiteralPath for file operations and explicit -Encoding UTF8 for text files. PowerShell 5.1 needs UTF-8 with BOM for .ps1 files containing non-ASCII text. Scripts above ~30000 encoded characters are rejected: save a .ps1 file with that encoding and invoke it with & 'path.ps1'.
