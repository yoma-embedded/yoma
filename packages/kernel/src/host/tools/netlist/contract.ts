...
- Netlists usually do NOT carry the MCU part number, but the USER'S REQUEST often does (prompt text, silkscreen, BOM). When stm32config is available and the STM32 part is known, pass \`part\` on the first call: you get the full board IR, including peripheral suggestions with per-signal evidence/confidence and a cfg_seed for stm32config. For non-STM32 controllers, or when stm32config is unavailable, omit \`part\` and use the raw map.
- Without \`part\` you get the raw per-pin connection map (pin → net → traced endpoints), tightly truncated: use the full output file when truncated. Use \`part\` only when local CubeMX resources are available.
- The board IR shares stm32config's resource preparation: it reads the user's local CubeMX database and builds a reusable device cache on demand. When CubeMX is missing or misconfigured, use the raw map without \`part\`; do not repeatedly attempt board IR or install CubeMX merely to use this tool. The raw map works without CubeMX for any MCU.
...
export const NETLIST_CONTRACT = {
  ...
  guidelines: [
    "Hardware bring-up starts from the schematic: run netlist first, including local Altium Smart PDFs. Pass part only when stm32config is available and an STM32 part is known; otherwise use the raw map. Do not read or send the whole PDF to the model. Treat low-confidence peripheral suggestions as hypotheses to verify.",
  ],
