...
- Exact register/bitfield/address/reset VALUES quoted in prose are contextual, not authoritative — read the register table itself with read_section before quoting a value; for chips the stm32config tool covers, its generated output is authoritative for configuration values.
- Which pad carries which signal — alternate functions, ADC channel numbers, timer channels, the package pinout — is NOT a manual question for a chip stm32config covers: \`stm32config describe-mcu\` answers it from the device database; read the full output file if its preview is truncated. Search here only for parts that tool does not cover (or when it reports no device data packs).
