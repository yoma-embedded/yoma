...
describe("子 agent 宿主(P2)", () => {
  ...
  test(
    ...
    async () => {
      ...
      const names = (context: LlmContext) => (context.tools ?? []).map((tool) => tool.name)
      ...
      const prompt = script.last("看看工具").systemPrompt ?? ""
  ...
  test(
    ...
    async () => {
      ...
      script.route(
        "派无人值守",
        (context) => {
          schema = JSON.stringify(context.tools?.find((tool) => tool.name === "agent")?.parameters)
          return fauxAssistantMessage([
