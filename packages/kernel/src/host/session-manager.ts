...
export class SessionManager {
  ...
  async prompt(sessionID: string, input: PromptInput): Promise<{ messageID: string; queued?: boolean }> {
    // 授权检查排在**一切副作用之前**:用户的每一句新话都是一次新的付费执行 —— 空闲时起一轮的、
    // 忙时排进收件箱的(它会把正在跑的那一轮续下去)都一样,所以查在分岔之前。手动压缩在飞时
    // admitPrompt 会先 stop() 它,而没有授权的新请求不该有本事打断任何已经被接受的操作。
    // 检查只在这里做一次 —— 通过之后这一轮(含轮内的自动压缩、重试、它派出去的子 agent 与子 agent
    // 完成通知叫醒的续轮)跑到自然结束,中途到期不回头查。
