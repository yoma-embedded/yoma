...
export function renderNoIndexHelp(): string {
	return [
		...
		"  tsx packages/kernel/src/host/domain/examples/cli.ts index --ecosystem esp-idf --root <esp-idf 检出目录>",
		"  tsx packages/kernel/src/host/domain/examples/cli.ts index --ecosystem stm32cube --root <STM32Cube 固件包目录>",
		"生成后重试本次检索。",
	].join("\n");
}
