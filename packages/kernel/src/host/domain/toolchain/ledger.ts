 ...
 * 这里**不 import** 它:工具间(host/domain)在依赖图里比会话间更底层(host 消费
 * domain 的工具集,resolve.ts 给 agent 子进程铺 PATH),反向 import 会成环,守门测试
 * boundary.test.ts 也不放行。默认值就地重新算一遍 —— 两边各自维护同一个常量、
