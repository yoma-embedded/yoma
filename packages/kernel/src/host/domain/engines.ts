/**
 * 引擎进程的共享辅助:路径解析 + 子进程运行 + 探针租约 + 输出预算。
 *
 * 2026-09-10 工具归零之后,本仓的活跃消费者是 core/la/engine.ts(yoma-la)与
 * kernel host 的 preflight.ts(用 ENGINE_BINARIES / exe 核对 bin 目录);重写中的各工具
 * 会重新接上 runEngine / 探针租约这两条。attic/tools/ 里的旧实现仍引用这一份,
 * 动 killTree / runEngine / 租约的人请连那边一起读 —— 被打坏的往往不是引擎调用,
 * 而是 gdb 的杀树链和 log 的采集子进程,也就是"孤儿 gdbserver 攥着探针不放,
 * 报错长得和没插板子一模一样"那条疤。整个文件经 src/index.ts 的 `export *`
 * 抬成了包的公共 API。
 *
