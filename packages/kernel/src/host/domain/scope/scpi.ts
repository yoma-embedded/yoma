...
export interface ScpiTransport {
	...
	write(bytes: Uint8Array, timeoutMs?: number, signal?: AbortSignal): Promise<void>;
...
export class TcpScpiTransport implements ScpiTransport {
	...
	static connect(host: string, port: number, timeoutMs = 3000, signal?: AbortSignal): Promise<TcpScpiTransport> {
		if (signal?.aborted) return Promise.reject(abortError(signal));
		return new Promise((resolve, reject) => {
			const socket = net.connect({ host, port });
			const cancel = () => { socket.destroy(); reject(abortError(signal)); };
			signal?.addEventListener("abort", cancel, { once: true });
			socket.once("close", () => { clearTimeout(timer); signal?.removeEventListener("abort", cancel); });
	...
	async write(bytes: Uint8Array, timeoutMs = 5000, signal?: AbortSignal): Promise<void> {
		...
		await new Promise<void>((resolve, reject) => {
			...
			const cancel = () => { this.socket.destroy(); finish(abortError(signal)); };
			const timer = setTimeout(() => { this.socket.destroy(); finish(new ScpiTimeoutError("scope: write deadline exceeded")); }, timeoutMs);
			signal?.addEventListener("abort", cancel, { once: true });
...
export interface UsbDeviceLike {
	...
	controlTransferIn(setup: { requestType: "standard" | "class" | "vendor"; recipient: "device" | "interface" | "endpoint" | "other"; request: number; value: number; index: number }, length: number, timeout?: number): Promise<{ status: string; data?: DataView }>;
...
function usbName(device: UsbDeviceLike, key: "serialNumber" | "productName"): string | undefined {
	// node-usb may open a temporary handle to fetch an uncached string descriptor.
	// Driver/permission failures must not hide the device from discovery.
...
function deadlineAfter(timeoutMs: number): number {
	if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) throw new Error("scope: timeout must be positive and finite");
	return Date.now() + timeoutMs;
}

function remaining(deadline: number): number {
	const ms = Math.ceil(deadline - Date.now());
	if (ms <= 0) throw new ScpiTimeoutError("scope: operation deadline exceeded");
	return ms;
}
...
export class UsbTmcTransport implements ScpiTransport {
	...
	/** Device seam for deterministic USBTMC tests; production always discovers with open(). */
	static async fromDevice(device: UsbDeviceLike, signal?: AbortSignal): Promise<UsbTmcTransport> {
		if (signal?.aborted) throw abortError(signal);
		try {
			await device.open();
			if (signal?.aborted) throw abortError(signal);
			...
			throw new Error(`scope: cannot open Siglent instrument on USB: ${msg}. ${usbOpenHint()}`);
		}
		let cfg: UsbDeviceLike["configuration"];
		try { cfg = device.configuration; }
		catch (error) { await device.close().catch(() => undefined); throw error; }
		const itf = cfg?.interfaces.find((i) => i.alternate.interfaceClass === 0xfe && i.alternate.interfaceSubclass === 3);
		if (!itf) {
			await device.close().catch(() => undefined);
			throw new Error("scope: device has no USBTMC interface");
		}
		...
		try {
			...
			await device.releaseInterface(itf.interfaceNumber).catch(() => undefined);
			await device.close().catch(() => undefined);
			const msg = error instanceof Error ? error.message : String(error);
			throw new Error(`scope: cannot claim the USBTMC interface: ${msg}. ${usbOpenHint()}`);
		}
		return new UsbTmcTransport(device, itf.interfaceNumber, bulkOut.endpointNumber, bulkIn.endpointNumber, usbName(device, "serialNumber"), usbName(device, "productName"));
	}
	...
	private async io<T>(run: () => Promise<T>, signal?: AbortSignal): Promise<T> {
		if (signal?.aborted) throw abortError(signal);
		if (this.closing) throw new Error(`scope: connection ${this.label} is closed`);
		// node-usb-rs close/release drop the handles, but an in-flight native transfer owns an
		// endpoint clone. Closing is not proof that it stopped: still await its finite native
		// timeout (the screenshot path can take 15 s). Never race and release the instrument
		// lease early, leaving that transfer to consume the next query's response.
	...
	async write(bytes: Uint8Array, timeoutMs = 5000, signal?: AbortSignal): Promise<void> {
		...
		buf[8] = 1; // EOM
		buf.set(bytes, 12);
		const sent = await this.io(() => this.device.nativeTransferOut(this.epOut, timeoutMs, buf), signal);
		if (sent !== buf.length) throw new Error(`scope: short USB write (${sent} of ${buf.length} bytes)`);
	}
	...
	async read(timeoutMs: number, signal?: AbortSignal): Promise<Uint8Array> {
		...
		for (let i = 0; ; i++) {
			...
			const sent = await this.io(() => this.device.nativeTransferOut(this.epOut, remaining(deadline), req), signal);
			if (sent !== req.length) throw new Error("scope: short USBTMC read request");
			let data: Uint8Array | null;
			try {
				data = await this.io(() => this.device.nativeTransferIn(this.epIn, remaining(deadline), USBTMC_CHUNK + 12), signal);
				remaining(deadline);
			} catch (error) {
				if (signal?.aborted) throw abortError(signal);
				if (error instanceof ScpiTimeoutError) throw error;
				const msg = error instanceof Error ? error.message : String(error);
			...
			if (!data) throw new ScpiTimeoutError(`scope: no response over ${this.label} within ${timeoutMs} ms`);
			if (data.length < 12) throw new Error("scope: short USBTMC header from the instrument");
			if (data[0] !== USBTMC_REQUEST_DEV_DEP_MSG_IN || data[1] !== tag || data[2] !== (~tag & 0xff)) {
				throw new Error("scope: USBTMC response has an invalid message ID or transaction tag");
			}
			const size = new DataView(data.buffer, data.byteOffset, data.byteLength).getUint32(4, true);
			if (size > USBTMC_CHUNK) throw new Error("scope: USBTMC response exceeds requested byte count");
			if (size === 0 && (data[8]! & 1) === 0) throw new Error("scope: USBTMC response made no progress");
			if (total + size > MAX_RESPONSE_BYTES) throw new Error("scope: USBTMC response exceeds the memory limit");
	...
	async clear(): Promise<number> {
		if (this.closing) throw new Error(`scope: connection ${this.label} is closed`);
		const deadline = deadlineAfter(1000);
		try {
			await this.device.controlTransferIn({ requestType: "class", recipient: "interface", request: USBTMC_INITIATE_CLEAR, value: 0, index: this.iface }, 1, 100);
			for (let i = 0; i < 50; i++) {
				const r = await this.device.controlTransferIn({ requestType: "class", recipient: "interface", request: USBTMC_CHECK_CLEAR_STATUS, value: 0, index: this.iface }, 2, Math.min(100, remaining(deadline)));
...
function usbOpenHint(): string {
	switch (process.platform) {
		case "win32":
			return "On Windows direct USB access requires a WinUSB driver (for example via Zadig), replacing the Siglent/NI USBTMC driver for this instrument. Check its current driver before changing it.";
		case "linux":
...
export class ScpiClient {
	...
	private async rawWrite(command: string, deadline = deadlineAfter(5000), signal?: AbortSignal): Promise<void> {
	...
	private async fill(timeoutMs: number, signal?: AbortSignal): Promise<void> {
		...
		if (this.buf.length + chunk.length > MAX_RESPONSE_BYTES) throw new Error("scope: response exceeds the memory limit");
	...
	private async drainLocked(idleMs = 300): Promise<number> {
		const deadline = deadlineAfter(2500);
		let dropped = this.buf.length;
		this.buf = new Uint8Array(0);
		dropped += await this.transport.clear();
		for (let i = 0; i < 64; i++) {
			// A drain succeeds only after a whole quiet interval, never after a byte/time budget ran out.
			if (remaining(deadline) < idleMs) throw new ScpiTimeoutError("scope: response queue did not become quiet while clearing");
			try {
				const chunk = await this.transport.read(idleMs, this.lifetime.signal);
		...
		throw new ScpiTimeoutError("scope: response queue exceeded the clearing budget");
	}
		...
		return this.locked(async () => {
			...
			try {
				for (;;) {
					...
					await this.fill(remaining(deadline), signal);
				}
			} catch (error) {
	...
	queryBlock(command: string, options: ScpiQueryOptions & { headerTimeoutMs?: number } = {}): Promise<Uint8Array> {
		return this.locked(async () => {
			const deadline = deadlineAfter(options.timeoutMs ?? 30_000);
			const signal = options.signal ? AbortSignal.any([options.signal, this.lifetime.signal]) : this.lifetime.signal;
			await this.prepare(signal);
			this.skipNewlines();
			await this.rawWrite(command, deadline, signal);
			...
			try {
				...
				for (;;) {
					...
					await this.fill(Math.min(headerTimeout, remaining(deadline)), signal);
				}
				const { start, digits, length } = header;
				if (length > MAX_RESPONSE_BYTES) throw new Error("scope: binary response exceeds the memory limit");
				...
				while (this.buf.length < need) await this.fill(Math.min(dataTimeout, remaining(deadline)), signal);
				this.take(payloadAt);
	...
	async close(): Promise<void> {
		...
		this.closed = true;
		this.lifetime.abort(new Error(`scope: connection ${this.label} is closed`));
