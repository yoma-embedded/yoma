...
export class SiglentScope {
	...
	static async open(address: ScpiAddress | string, options: { connectTimeoutMs?: number; signal?: AbortSignal } = {}): Promise<SiglentScope> {
	...
	async setChannel(spec: ChannelSpec, signal?: AbortSignal): Promise<Applied<ChannelState>> {
		...
		if (spec.vdiv !== undefined && !nearly(state.vdiv, spec.vdiv)) mismatches.push(`C${n} vdiv: asked ${fmt(spec.vdiv)}, scope reports ${fmt(state.vdiv)} ${state.unit}/div (1-2-5 steps; range depends on probe setting)`);
	...
	async setTrigger(spec: TriggerSpec, signal?: AbortSignal): Promise<Applied<TriggerState>> {
		...
		if (spec.level !== undefined && !nearly(state.level, spec.level, 5e-3, 1e-4)) mismatches.push(`trigger level: asked ${fmt(spec.level)}, scope reports ${fmt(state.level)} (source channel units; range is about ±4.1 divisions of the source channel)`);
	 ...
	 */
	async readWaveform(ch: number, options: { maxPoints?: number; stride?: number; signal?: AbortSignal; timeoutMs?: number } = {}): Promise<Waveform> {
		...
		for (let i = 0; i < maxWindows && got < expected && srcPos < recordPoints; i++) {
			...
			if (codes.length !== windowPoints) throw new Error(`scope: incomplete C${n} waveform window at ${srcPos}: expected ${windowPoints} samples, received ${codes.length}`);
