...
export function parseWaveDesc(bytes: Uint8Array): WaveDesc {
	...
	if (result.commType !== 0 && result.commType !== 1) throw new Error(`scope: unsupported waveform sample format ${result.commType}`);
	if (result.descriptorLength < WAVEDESC_LENGTH || result.descriptorLength > bytes.length) throw new Error("scope: incomplete waveform descriptor");
	for (const [name, value] of Object.entries({ waveArrayCount: result.waveArrayCount, waveArray1Bytes: result.waveArray1Bytes, firstPoint: result.firstPoint })) {
		if (!Number.isSafeInteger(value) || value < 0) throw new Error(`scope: invalid ${name} in waveform descriptor`);
	}
	for (const [name, value] of Object.entries({ verticalGain: result.verticalGain, codePerDiv: result.codePerDiv, horizInterval: result.horizInterval, probe: result.probe })) {
...
export function decodeSamples(payload: Uint8Array, commType: number): Int16Array | Int8Array {
	if (commType !== 0 && commType !== 1) throw new Error(`scope: unsupported waveform sample format ${commType}`);
	if (commType === 1) {
		if (payload.length % 2 !== 0) throw new Error("scope: incomplete WORD waveform (odd byte count)");
