# 上游关系:`packages/kernel/src/host` ← pi `packages/coding-agent`
...
> 2026-09-10 两件事。**工具归零**:下面提到的 `core/tools/**` 与独立 CLI(`src/cli/`)整体退役 ——
...
- 删掉了上游的 153 个文件:TUI / CLI / `modes/{interactive,rpc,...}` / `extensions/` / `core/agent-session.ts` /
  ...
  yoma 当年在 `kernel/src/host/` 重建了只服务桌面端的最小子集;2026-09-10 切到 `pi-agent-core` 之后
  **压缩与重试交回内核**(`host/compaction.ts` / `host/retry.ts` 已删,`lane.drive({ waitForRetry: true })`
  自带退避),这一层只剩 `src/host/session-manager.ts` —— 要找上游参照(比如溢出压缩策略)去
  `D:\toy\pi\packages\coding-agent\src\core\agent-session.ts`,不要被"内核只给了机制"这句话误导成上游没有。
- `src/host/models.ts` **同时是桌面端与 bench 两个消费方的模型目录来源**(经 `@yoma-desktop/kernel/host/models`
  这道叶子门可达,不再靠别名)。目录本身来自 pi-ai 的 `builtinProviders()`(2026-08-23 起,从前是两家的手写表);这个文件
...
> **2026-09-10 起本节是历史**:kernel 切到 `@earendil-works/pi-agent-core` 之后,四件套直接用内核的
> `createBashTool` / `createReadTool` / `createEditTool` / `createWriteTool`(就是下面说的那份 headless 版),
> 自有四件套随工具归零进了 `attic/tools/`。下面记的是当年不换的理由,重写工具时仍值得读一遍。
