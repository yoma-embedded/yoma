/**
 * la(逻辑分析仪)纯模块的测试 —— zip / dsl / annotations / model。除了 .dsl 夹具
 * (仓内的 DSView demo 会话)之外不碰引擎、不碰硬件,断言全是确定值:边沿数、列位图、
 * 事务聚合、渲染文本、expect 差分。
 *
 * 2026-09-10 工具归零:原先同文件第二层那组"真跑 engines/bin/yoma-la"的端到端用例
 * 随 la 工具搬到了 attic/test/la-tool.test.ts,不再编译也不再跑。
 *
...
} from "../src/host/domain/la/annotations.ts";
import {
	DslFile,
	type EdgeList,
	channelStats,
	...
	parseDslHeader,
	parseSizeString,
} from "../src/host/domain/la/dsl.ts";
import { expectDiff, groupI2c, groupSpi, groupUart, protocolOf, renderEvents } from "../src/host/domain/la/model.ts";
import { ZipFile } from "../src/host/domain/la/zip.ts";
