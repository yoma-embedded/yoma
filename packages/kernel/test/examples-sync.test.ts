...
import {
	...
	upsertSource,
	writeCorpusMarker,
} from "../src/host/domain/examples/store.ts";
import { type CodelibMeta, listRemoteCorpora, syncCorpus, syncIndex } from "../src/host/domain/examples/sync.ts";
...
describe("第一级:syncIndex", () => {
	test("落地索引+富化并记账(root 空、remote 填服务器)", async () => {
		...
		// 落地的索引能被现有读侧认出来。
		const { readIndexFile } = await import("../src/host/domain/examples/store.ts");
