...
import {
	...
	si,
	waveStats,
} from "../src/host/domain/scope/analyze.ts";
import {
	COUPLING_ENUM,
	TIMEBASE_ENUM,
	WAVEDESC_LENGTH,
	...
	type VoltScale,
	voltScaleOf,
} from "../src/host/domain/scope/preamble.ts";
import {
	type ScpiClient,
	ScpiTimeoutError,
	bmpComplete,
...
describe("WAVEDESC 与换算", () => {
	...
	it("损坏比例和时间字段不能变成看似有效的测量", () => {
		for (const [offset, value] of [[156, Number.NaN], [164, 0], [176, -1], [328, Infinity]]) {
			const bytes = blockPayload(PREAMBLE_RESPONSE).slice();
			new DataView(bytes.buffer).setFloat32(offset!, value!, true);
			expect(() => parseWaveDesc(bytes)).toThrow(/invalid/);
		}
		expect(() => decodeSamples(new Uint8Array([1, 2, 3]), 1)).toThrow(/odd byte count/);
		expect(() => decodeSamples(new Uint8Array([1, 2]), 9)).toThrow(/unsupported/);
	});
...
describe("波形分析", () => {
	...
	it("确定性宽带噪声:保留幅值和边沿统计,不将随机穿越报成频率或占空比", () => {
		...
		const s = waveStats(noise, SQUARE_SCALE, INTERVAL);
		expect(s.freq).toBeUndefined();
		expect(s.period).toBeUndefined();
		expect(s.duty).toBeUndefined();
		expect(s.edges).toBeGreaterThan(1000);
		expect(s.pp).toBeGreaterThan(1.9);
		expect(Math.abs(s.mean)).toBeLessThan(0.05);
		expect(s.rms).toBeGreaterThan(0.5);
	});

	it("不等间隔脉冲:有多个清楚边沿也不能报一个周期", () => {
		const pulses = new Int16Array(1300).fill(-1000);
		for (const start of [20, 120, 280, 530, 610, 920, 1040]) pulses.fill(1000, start, start + 30);
		const s = waveStats(pulses, SQUARE_SCALE, INTERVAL);
		expect(s.edges).toBe(14);
		expect(s.freq).toBeUndefined();
		expect(s.period).toBeUndefined();
		expect(s.duty).toBeUndefined();
		expect(s.rms).toBe(1);
	...
	it("方波带少量周期抖动:仍保留频率和占空比估计", () => {
		const pulses = new Int16Array(8500).fill(-1000);
		let start = 20;
		for (const period of [1000, 1003, 998, 1002, 997, 1001, 999, 1000]) {
			pulses.fill(1000, start, start + 500);
			start += period;
		}
		const s = waveStats(pulses, SQUARE_SCALE, INTERVAL);
		expect(s.freq).toBeGreaterThan(990);
		expect(s.freq).toBeLessThan(1010);
		expect(s.period).toBeCloseTo(1e-3, 5);
		expect(s.duty).toBeCloseTo(0.5, 2);
	});

	it("周期不足三次:保留已看到的边沿,不外推稳定频率", () => {
		const s = waveStats(square.subarray(0, 2500), SQUARE_SCALE, INTERVAL);
	...
	it("parseNumber:NR3 / 四星 / 带前缀", () => {
		...
		expect(parseNumber("+1.25E-3")).toBe(0.00125);
		expect(parseNumber("1.2garbage")).toBeNull();
		expect(parseNumber("1e999")).toBeNull();
	});
});

describe("scope 目录的命名空间导出", () => {
	it("scope 这一坨能从 domain/scope/index.ts 拿到", () => {
...
describe("SiglentScope(对着假 SDS)", () => {
	...
	it("setChannel:1 V/A 电流探头的 A 单位贯穿状态和波形,数值不二次缩放", async () => {
		...
		try {
			...
			expect(result.state).toMatchObject({ unit: "A", probe: 1, vdiv: 0.1 });
			expect(fake.log).toContain(":CHANnel3:UNIT A");
			const wave = await scope.readWaveform(3, { maxPoints: 100 });
			expect(wave.unit).toBe("A");
			expect(wave.probe).toBe(1);
			// FakeSds 的冻结记录仍用真机 preamble,设置显示量程不会重新采集。
			expect(wave.codes).toEqual(before.codes);
			expect(codeToVolts(wave.codes[0]!, wave.scale)).toBe(codeToVolts(before.codes[0]!, before.scale));
		} finally {
			await scope.setChannel({ ch: 3, on: false, unit: "V" });
		}
	});

	it("setChannel:仪器静默拒绝单位切换时必须报告实际单位", async () => {
		const command = scope.client.command.bind(scope.client);
		const write = vi.spyOn(scope.client, "command").mockImplementation((value, options) =>
			value === ":CHANnel3:UNIT A" ? Promise.resolve() : command(value, options));
		try {
			const result = await scope.setChannel({ ch: 3, unit: "A" });
			expect(result.state.unit).toBe("V");
			expect(result.mismatches).toEqual(["C3 unit: asked A, scope reports V"]);
	...
	it("无效采集预算/设置在写入任何命令前拒绝", async () => {
		await flush();
		fake.clearLog();
		await expect(scope.readWaveform(1, { maxPoints: Number.NaN })).rejects.toThrow(/maxPoints/);
		await expect(scope.readWaveform(1, { stride: 0 })).rejects.toThrow(/stride/);
		await expect(scope.setChannel({ ch: 1, on: false, probe: Infinity })).rejects.toThrow(/probe/);
		await expect(scope.setChannel({ ch: 1, on: false, unit: "AMP" as "A" })).rejects.toThrow(/unit/);
		await expect(scope.setTimebase({ scale: -1 })).rejects.toThrow(/timebase/);
		await expect(scope.setTrigger({ level: Number.NaN })).rejects.toThrow(/trigger/);
		await flush();
		expect(fake.log).toEqual(["*OPC?"]);
	});

	it("半包波形不能继续拼凑成一次成功采集", async () => {
		const original = scope.client.queryBlock.bind(scope.client);
		const query = vi.spyOn(scope.client, "queryBlock").mockImplementation(async (command, options) => {
	...
	it("measure:参数校验", async () => {
		await expect(scope.measure([{ type: "FREQ", source: "LINE" }])).rejects.toThrow(/measurement source/);
	...
	it("measure:仪器拒绝测量源时返回实际读回的源并点名差异", async () => {
		const original = scope.client.query.bind(scope.client);
		const query = vi.spyOn(scope.client, "query").mockImplementation((command, options) =>
			command === ":MEASure:ADVanced:P1:SOURce?" ? Promise.resolve("C2") : original(command, options));
		try {
			const result = await scope.measure([{ type: "FREQ", source: "C1" }]);
			expect(result.results[0]!.source).toBe("C2");
			expect(result.mismatches).toContain("P1: asked source C1, scope reports C2");
		} finally { query.mockRestore(); }
	});
...
describe("readWaveform 的窗口语义", () => {
	it("配置点数变小也不截断已经冻结的完整记录", async () => {
		const fake = await FakeSds.start({ recordPoints: 1000, configuredPoints: 100, maxPoint: 300 });
		const scope = await SiglentScope.open(fake.address);
		try {
			const waveform = await scope.readWaveform(1, { stride: 1 });
			expect(waveform.recordPoints).toBe(1000);
			expect(waveform.codes.length).toBe(1000);
		} finally { await scope.close(); await fake.close(); }
	});
