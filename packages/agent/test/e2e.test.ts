...
// pi-minimal: shared collection; each faux provider gets a unique id so coexisting fakes route correctly.
const models = createModels();
let fauxCount = 0;

function createFauxRegistration(options: RegisterFauxProviderOptions = {}): FauxProviderHandle {
...
async function basicPrompt(model: Model<string>) {
	const agent = new Agent({
		streamFn: (m, ctx, opts) => models.streamSimple(m, ctx, opts), // pi-minimal: Agent's default streamFn now throws
		initialState: {
