...
/**
 * 用 Buffer 独立实现一遍尾部截断,作为差分测试的对照组。
 *
 * 注意只用 `Buffer.from`,不用 `Buffer.byteLength` —— 后者在 Bun 上对孤立代理项少算 1 字节
 * (见 truncate.ts 里 utf8ByteLength 的注释),拿它当基准会把对照组本身也算错。
 */
...
function assertMatchesBufferTail(input: string, maxByteValues?: readonly number[]): void {
	...
	for (const maxBytes of values) {
		...
		const outputBytes = encodedLength(result.content);
...
describe("truncate utilities", () => {
	...
	it("matches Buffer tail truncation semantics for surrogate edge cases", () => {
		// 孤立代理项:JS 字符串允许,UTF-8 编码时必须换成 U+FFFD。
