import { dict as en } from "./en"

type Keys = keyof typeof en

export const dict = {
  "ui.sessionReview.selection.line": "第 {{line}} 行",
  "ui.sessionReview.selection.lines": "第 {{start}}-{{end}} 行",
  "ui.fileMedia.kind.image": "图片",
  "ui.fileMedia.kind.audio": "音频",
  "ui.fileMedia.state.removed": "{{kind}}已移除",
  "ui.fileMedia.state.loading": "正在加载{{kind}}...",
  "ui.fileMedia.state.error": "加载{{kind}}失败",
  "ui.fileMedia.state.unavailable": "{{kind}}不可预览",
  "ui.fileMedia.binary.title": "二进制文件",
  "ui.fileMedia.binary.description.path": "无法显示 {{path}}，因为它是二进制文件。",
  "ui.fileMedia.binary.description.default": "无法显示此文件，因为它是二进制文件。",

  "ui.lineComment.label.prefix": "评论 ",
  "ui.lineComment.label.suffix": "",
  "ui.lineComment.editorLabel.prefix": "正在评论 ",
  "ui.lineComment.editorLabel.suffix": "",
  "ui.lineComment.placeholder": "添加评论",
  "ui.lineComment.submit": "评论",

  "ui.sessionTurn.steps.show": "显示步骤",
  "ui.sessionTurn.steps.hide": "隐藏步骤",
  "ui.sessionTurn.summary.response": "回复",
  "ui.sessionTurn.diff.showMore": "显示更多更改（{{count}}）",

  "ui.sessionTurn.retry.retrying": "重试中",
  "ui.sessionTurn.retry.inSeconds": "{{seconds}} 秒后",
  "ui.sessionTurn.retry.attempt": "第 {{attempt}} 次",
  "ui.sessionTurn.retry.attemptLine": "{{line}} - 第 {{attempt}} 次",
  "ui.sessionTurn.retry.geminiHot": "gemini 当前过载",
  "ui.sessionTurn.error.freeUsageExceeded": "免费使用额度已用完",
  "ui.sessionTurn.error.addCredits": "添加积分",

  "ui.sessionTurn.status.delegating": "正在委派工作",
  "ui.sessionTurn.status.planning": "正在规划下一步",
  "ui.sessionTurn.status.gatheringContext": "正在探索",
  "ui.sessionTurn.status.gatheredContext": "已探索",
  "ui.sessionTurn.status.compacting": "正在压缩上下文",
  "ui.sessionTurn.status.searchingCodebase": "正在搜索代码库",
  "ui.sessionTurn.status.searchingWeb": "正在搜索网页",
  "ui.sessionTurn.status.makingEdits": "正在修改",
  "ui.sessionTurn.status.runningCommands": "正在运行命令",
  "ui.sessionTurn.status.thinking": "思考中",
  "ui.sessionTurn.status.waitingModel": "等待模型",
  "ui.sessionTurn.status.runningTools": "正在运行 {{names}}",
  "ui.sessionTurn.status.runningToolsMore": "正在运行 {{names}} 等 {{total}} 个",
  "ui.sessionTurn.status.toolSeparator": "、",
  "ui.sessionTurn.status.waitingConfirm": "等待确认 {{tool}}",
  "ui.sessionTurn.status.thoughtChars": "已想 {{chars}} 字",
  "ui.sessionTurn.status.thinkingWithTopic": "思考：{{topic}}",
  "ui.sessionTurn.status.gatheringThoughts": "正在整理思路",
  "ui.sessionTurn.status.consideringNextSteps": "正在考虑下一步",

  "ui.messagePart.questions.dismissed": "问题已忽略",
  "ui.messagePart.compaction": "会话已压缩",
  "ui.messagePart.context.read.one": "{{count}} 次读取",
  "ui.messagePart.context.read.other": "{{count}} 次读取",
  "ui.messagePart.context.search.one": "{{count}} 次搜索",
  "ui.messagePart.context.search.other": "{{count}} 次搜索",
  "ui.messagePart.context.list.one": "{{count}} 个列表",
  "ui.messagePart.context.list.other": "{{count}} 个列表",
  "ui.messagePart.context.failed": "{{count}} 次失败",
  "ui.processGroup.title": "处理详情",
  "ui.processGroup.tools.one": "{{count}} 次工具调用",
  "ui.processGroup.tools.other": "{{count}} 次工具调用",
  "ui.processGroup.failed": "{{count}} 次失败",
  "ui.processGroup.unfinished": "{{count}} 次未完成",
  "ui.processGroup.expand": "展开处理详情",
  "ui.processGroup.collapse": "收起处理详情",
  "ui.reasoning.title": "思考",
  "ui.messagePart.diagnostic.error": "错误",
  "ui.messagePart.title.edit": "编辑",
  "ui.messagePart.title.write": "写入",
  "ui.messagePart.option.typeOwnAnswer": "输入自己的答案",
  "ui.messagePart.review.title": "检查你的答案",

  "ui.list.loading": "加载中",
  "ui.list.empty": "无结果",
  "ui.list.clearFilter": "清除筛选",
  "ui.list.emptyWithFilter.prefix": "没有关于",
  "ui.list.emptyWithFilter.suffix": "的结果",

  "ui.messageNav.newMessage": "新消息",

  "ui.textField.copyToClipboard": "复制到剪贴板",
  "ui.textField.copyLink": "复制链接",
  "ui.textField.copied": "已复制",

  "ui.imagePreview.alt": "图片预览",
  "ui.scrollView.ariaLabel": "可滚动内容",

  "ui.tool.read": "读取",
  "ui.tool.loaded": "已加载",
  "ui.tool.grep": "Grep",
  "ui.tool.shell": "Shell",
  "ui.tool.todos": "待办",
  "ui.tool.todos.read": "读取待办",
  "ui.tool.questions": "问题",
  "ui.tool.agent": "{{type}} 智能体",
  "ui.tool.agent.default": "智能体",
  "ui.tool.exitCode": "退出码",

  "ui.tool.stm32config": "STM32 配置",

  "ui.tool.netlist": "网表",
  "ui.tool.netlist.mode.map": "引脚图",
  "ui.tool.netlist.mode.board_ir": "板级 IR",
  "ui.tool.netlist.part": "器件",
  "ui.tool.netlist.file.boardIr": "板级 IR",
  "ui.tool.netlist.file.stm32Map": "STM32 引脚映射",
  "ui.tool.netlist.file.cfgSeed": "配置种子",

  "ui.tool.flash": "烧录",
  "ui.tool.flash.command": "命令",
  "ui.tool.flash.recorded": "已记录镜像",

  "ui.tool.datasheet": "数据手册",
  "ui.tool.datasheet.action.search": "搜索",
  "ui.tool.datasheet.action.read_section": "读取章节",
  "ui.tool.datasheet.action.view_figure": "查看插图",
  "ui.tool.datasheet.hits.one": "{{count}} 条匹配",
  "ui.tool.datasheet.hits.other": "{{count}} 条匹配",
  "ui.tool.datasheet.hits.empty": "没有匹配结果",
  "ui.tool.datasheet.page": "第 {{page}} 页",
  "ui.tool.datasheet.section": "章节",
  "ui.tool.datasheet.level": "第 {{level}} 级",
  "ui.tool.datasheet.lines": "第 {{lines}} 行",
  "ui.tool.datasheet.chars": "{{count}} 字符",
  "ui.tool.datasheet.sections": "{{count}} 个章节",
  "ui.tool.datasheet.truncated": "已截断",
  "ui.tool.datasheet.parsedFrom": "解析自 {{path}}",

  "ui.tool.log": "串口日志",
  "ui.tool.log.action.start": "启动",
  "ui.tool.log.action.read": "读取",
  "ui.tool.log.action.wait": "等待",
  "ui.tool.log.action.status": "状态",
  "ui.tool.log.action.stop": "停止",
  "ui.tool.log.action.ports": "串口列表",
  "ui.tool.log.running": "运行中",
  "ui.tool.log.stopped": "已停止",
  "ui.tool.log.cursor": "读取位置",
  "ui.tool.log.totalLines": "总行数",
  "ui.tool.log.dropped": "已丢弃",
  "ui.tool.log.matched": "已匹配",
  "ui.tool.log.notMatched": "未匹配",

  "ui.tool.gdb": "GDB",
  "ui.tool.gdb.action.start": "启动",
  "ui.tool.gdb.action.break": "断点",
  "ui.tool.gdb.action.exec": "执行",
  "ui.tool.gdb.action.eval": "求值",
  "ui.tool.gdb.action.status": "状态",
  "ui.tool.gdb.action.stop": "停止",
  "ui.tool.gdb.state.halted": "已暂停",
  "ui.tool.gdb.state.running": "运行中",
  "ui.tool.gdb.state.exited": "已退出",
  "ui.tool.gdb.state.connection-lost": "连接已断开",
  "ui.tool.gdb.state.no-session": "无会话",
  "ui.tool.gdb.connection": "连接",
  "ui.tool.gdb.location": "停在",

  "ui.tool.la": "逻辑分析仪",
  "ui.tool.la.action.devices": "设备",
  "ui.tool.la.action.capture": "采集",
  "ui.tool.la.action.arm": "武装",
  "ui.tool.la.action.collect": "收取",
  "ui.tool.la.action.stop": "停止",
  "ui.tool.la.action.import": "导入",
  "ui.tool.la.action.list": "列表",
  "ui.tool.la.action.decoders": "解码器目录",
  "ui.tool.la.action.summary": "摘要",
  "ui.tool.la.action.decode": "解码",
  "ui.tool.la.action.events": "事件",
  "ui.tool.la.action.timing": "时序",
  "ui.tool.la.action.expect": "期望比对",
  "ui.tool.la.capture": "采集",
  "ui.tool.la.samplerate": "采样率",
  "ui.tool.la.samples": "采样点",
  "ui.tool.la.duration": "时长",
  "ui.tool.la.channels": "通道",
  "ui.tool.la.decoders": "解码器",
  "ui.tool.la.annotations": "{{count}} 条注解",
  "ui.tool.la.issues": "不符",
  "ui.tool.la.armed": "已武装",
  "ui.tool.la.timedOut": "触发超时",
  "ui.tool.la.truncated": "已截断",
  "ui.tool.scope": "示波器",
  "ui.tool.scope.action.connect": "连接",
  "ui.tool.scope.action.status": "状态",
  "ui.tool.scope.action.setup": "设置",
  "ui.tool.scope.action.capture": "采集",
  "ui.tool.scope.action.arm": "武装",
  "ui.tool.scope.action.collect": "收取",
  "ui.tool.scope.action.measure": "测量",
  "ui.tool.scope.action.samples": "采样",
  "ui.tool.scope.action.screenshot": "截图",
  "ui.tool.scope.action.list": "列表",
  "ui.tool.scope.action.raw": "SCPI",
  "ui.tool.scope.instrument": "仪器",
  "ui.tool.scope.address": "地址",
  "ui.tool.scope.capture": "采集",
  "ui.tool.scope.timebase": "时基",
  "ui.tool.scope.sampleRate": "采样率",
  "ui.tool.scope.points": "点数",
  "ui.tool.scope.mdepth": "存储深度",
  "ui.tool.scope.trigger": "触发",
  "ui.tool.scope.channels": "通道",
  "ui.tool.scope.measurements": "测量",
  "ui.tool.scope.armed": "已武装",
  "ui.tool.scope.timedOut": "超时",
  "ui.tool.scope.truncated": "已截断",

  "ui.common.file.one": "个文件",
  "ui.common.file.other": "个文件",
  "ui.common.question.one": "个问题",
  "ui.common.question.other": "个问题",

  "ui.common.add": "添加",
  "ui.common.back": "返回",
  "ui.common.cancel": "取消",
  "ui.common.confirm": "确认",
  "ui.common.dismiss": "忽略",
  "ui.common.close": "关闭",
  "ui.common.next": "下一步",
  "ui.common.submit": "提交",

  "ui.message.expand": "展开消息",
  "ui.message.collapse": "收起消息",
  "ui.message.copy": "复制",
  "ui.message.copyMessage": "复制消息",
  "ui.message.forkMessage": "分叉到新会话",
  "ui.message.revertMessage": "重置到此点",
  "ui.message.copyResponse": "复制回复",
  "ui.message.copied": "已复制！",
  "ui.message.interrupted": "已中断",
  "ui.message.queued": "排队中",
  "ui.message.attachment.alt": "附件",

  "ui.patch.action.deleted": "已删除",
  "ui.patch.action.created": "已创建",
  "ui.patch.action.moved": "已移动",
  "ui.patch.action.patched": "已应用补丁",

  "ui.question.subtitle.answered": "{{count}} 已回答",
  "ui.question.answer.none": "(无答案)",
  "ui.question.review.notAnswered": "(未回答)",
  "ui.question.multiHint": "可多选",
  "ui.question.singleHint": "选择一个答案",
  "ui.question.custom.placeholder": "输入你的答案...",

  "ui.fileSearch.placeholder": "查找",
  "ui.fileSearch.previousMatch": "上一个",
  "ui.fileSearch.nextMatch": "下一个",
  "ui.fileSearch.close": "关闭搜索",
  "ui.tool.task": "任务",
  // ---- 嵌入式工具卡片(v5-cards)
  "ui.tool.hw.readout": "读数",
  "ui.tool.hw.output": "工具输出",
  "ui.tool.hw.rawLines": "{{count}} 行",
  "ui.tool.agent.label": "子 agent",
  "ui.tool.agent.state.pending": "排队中",
  "ui.tool.agent.state.running": "运行中",
  "ui.tool.agent.state.background": "后台运行中",
  "ui.tool.agent.state.completed": "完成",
  "ui.tool.agent.state.failed": "失败",
  "ui.tool.agent.state.killed": "已停止",
  "ui.tool.agent.toolUses": "{{count}} 次工具调用",
  "ui.tool.agent.last": "最近 {{tool}}",
  "ui.tool.agent.maxTurns": "到轮数上限",
  "ui.tool.agent.prompt": "任务书",
  "ui.tool.agent.result": "结果",
  "ui.tool.agent.noResult": "子 agent 跑完了,没有文字结果。",
  "ui.tool.agent.inBackground": "在后台运行,完成后结果会以通知回到这个会话。",
  "ui.tool.agent.detached": "在后台跑(进度在输入框上方那条子 agent 栏里);完成后结果会以通知回到这个会话。",
  "ui.tool.agent.facts": "运行数据",
  "ui.tool.agent.readout.agent": "类型",
  "ui.tool.agent.readout.turns": "轮数",
  "ui.tool.agent.readout.tools": "工具调用",
  "ui.tool.agent.readout.last": "最近工具",
  "ui.tool.agent.readout.elapsed": "用时",
  "ui.tool.agent.readout.tokens": "上下文 token",
  "ui.tool.agent.readout.log": "进度日志",
  "ui.tool.agent.open": "打开子会话",
  "ui.tool.agent.background": "转到后台",
  "ui.tool.agent.stop": "停止",

  "ui.tool.flash.done": "已烧录",
  "ui.tool.flash.aborted": "中断",
  "ui.tool.flash.exit": "退出码 {{code}}",
  "ui.tool.flash.evidence": "关键输出",
  "ui.tool.flash.wrote": "写入",
  "ui.tool.flash.program": "编程",
  "ui.tool.flash.verify": "校验",

  "ui.tool.log.capturing": "采集中",
  "ui.tool.log.lines": "{{count}} 行",
  "ui.tool.log.nothingNew": "没有新行",
  "ui.tool.log.ports": "串口",
  "ui.tool.log.excerpt": "日志节选",
  "ui.tool.log.skipped": "跳过前面 {{count}} 行",
  "ui.tool.log.source": "来源",
  "ui.tool.log.state": "状态",
  "ui.tool.log.fullLog": "全量日志:",

  "ui.tool.gdb.fault": "故障",
  "ui.tool.gdb.faultKind": "类型",
  "ui.tool.gdb.flags": "标志位",
  "ui.tool.gdb.address": "出事地址",
  "ui.tool.gdb.faultAt": "出事的那一行",
  "ui.tool.gdb.excFrame": "异常帧",
  "ui.tool.gdb.frames": "调用栈",
  "ui.tool.gdb.values": "取值",
  "ui.tool.gdb.breakpoints": "断点",
  "ui.tool.gdb.state": "目标",
  "ui.tool.gdb.sessionLog": "会话转录:",

  "ui.tool.la.preview": "波形",
  "ui.tool.la.previewNote": "缩略预览 —— 完整采集在磁盘上,右栏「调试」里可以缩放。",
  "ui.tool.la.trigger": "触发采样点",
  "ui.tool.la.window": "窗口",
  "ui.tool.la.device": "设备",
  "ui.tool.la.saved": "已存:",

  "ui.tool.scope.interval": "采样间隔",
  "ui.tool.scope.quality": "采样质量",
  "ui.tool.scope.unavailable": "量不出来",
  "ui.tool.scope.off": "关",
  "ui.tool.scope.saved": "已存:",
  "ui.tool.scope.zoom": "放大示波器截图",
  "ui.tool.scope.screenshot": "仪器截图",
  "ui.tool.scope.screenshotNote": "仪器截图。保存的波形可在右栏「调试」面板缩放、查看游标。",

  // 时间线里硬件卡片右上角那个按钮(log / gdb / la / scope;flash 没有面板所以没有它)。
  "ui.tool.openInPanel": "在面板中打开",
  "ui.basicTool.interrupted": "未完成",
  "ui.basicTool.interruptedNote": "这一轮结束时它还没有结果:被停止了,或者它在跑的时候 app 关掉了。",
  "ui.toolErrorCard.failed": "失败",
  "ui.toolErrorCard.copyError": "复制错误",
  "ui.message.duration.seconds": "{{count}}秒",
  "ui.message.duration.minutesSeconds": "{{minutes}}分 {{seconds}}秒",
} satisfies Partial<Record<Keys, string>>
