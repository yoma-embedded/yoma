 ...
 *
 * 落盘的只有"下次打开还该是这样"的那几位:开合、高度、当前页签、右栏当前仪器。
...
const DEFAULTS: ConsoleStored = { open: false, height: DEFAULT_HEIGHT, tab: "log" }
...
export function readConsoleState(raw: string | null | undefined): ConsoleStored {
  ...
  try {
    ...
    return {
      ...
      rail: isInstrumentId(value.rail) ? value.rail : undefined,
    }
  } catch {
    return { ...DEFAULTS }
  }
...
export const consoleUI = createRoot(() => {
  ...
  return {
    ...
    /** 点页签:已经开着且停在这一档时再点一下 = 收起(同 VS Code 的底栏)。 */
    select(tab: InstrumentId) {
      if (store.open && store.tab === tab) {
        this.close()
        return
      }
