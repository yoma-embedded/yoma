...
- **内核**(`packages/{ai,agent}` 两个上游包 + `packages/kernel/src/host`)—— agent 循环、会话树、
  ...
  `host/tools/<名字>/{contract.ts,session.ts}` 逐个重写,首个是 flash,2026-09-12 起有 grep / find / ls / powershell;示波器与例程库 2026-09-11
...
| `npm run typecheck` | turbo 跑全部 11 个包,再加根 `tsconfig.json` —— **必须常绿 11/11 + 根** |
| `npm run lint` | oxlint |
...
| `npm run e2e:paint -w packages/desktop` | 真窗口首屏 + 点一遍:Electron 跑构建产物 + 接 CDP,首页 / 会话页(含逻辑分析仪面板)/ 草稿页 / 手册库 / 调试台全点一遍,零 `exceptionThrown` 零 `console.error`(窗口会在屏幕上闪几秒,别去点它) |
...
- `packages/kernel` 有**两个**项目:`kernel`(`src/**/*.test.ts` —— 投影器不变式、事件流、
  ...
  配置在 `vitest.domain.config.ts`,`fileParallelism: false` **串行**跑)。根 `vitest.config.ts`
  里显式列了第二份;CI 的 Windows 岗跑的就是 `--project kernel-domain`。
